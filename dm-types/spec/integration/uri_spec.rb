...
describe DataMapper::Types::Fixtures::Bookmark do
  describe "without URI" do
    before :all do
      @uri = nil
      @model = DataMapper::Types::Fixtures::Bookmark.new(:title  => "Check this out",
                                                         :uri    => @uri,
                                                         :shared => false,
                                                         :tags   => ["misc"])
    ...
    it "can be found by uri" do
      DataMapper::Types::Fixtures::Bookmark.first(:uri => @uri).should == @model
    end
  ...
  describe "with a blank URI" do
    ...
    describe "when reloaded" do
      ...
      it "is loaded as URI object" do
        @model.uri.should be_an_instance_of(Addressable::URI)
      end

      it "has the same original URI" do
        @model.uri.to_s.should == @uri
      end
    end
  end
