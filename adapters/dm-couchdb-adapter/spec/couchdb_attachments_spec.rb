...
describe DataMapper::Model do
  ...
  it "should raise an error on models without attachments property" do
    lambda { Zoo.new.add_attachment('test.txt', 'text/plain', 'test string') }.should raise_error(ArgumentError, "Attachments require '  property :attachments, JsonObject, :field => :_attachments'")
    lambda { Zoo.new.get_attachment('test.txt') }.should raise_error(ArgumentError, "Attachments require '  property :attachments, JsonObject, :field => :_attachments'")
  end

  it "should not raise an error on models with attachments property" do
    Message.new.add_attachment('test.txt', 'text/plain', 'test string')
    Message.new.get_attachment('test.txt')
  end
  ...
  describe "#add_attachment" do
    ...
    it "should add inline attributes to new records" do
      @message = Message.new
      @message.add_attachment('test.txt', 'text/plain', 'test string')
