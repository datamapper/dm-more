  ...
  module Validate
    module Fixtures
      ...
      class UDPPacket
        ...
        validates_present :checksum, :if => Proc.new { |packet| packet.underlying_ip_version == 6 }, :message => "Checksum is mandatory when used with IPv6"
        validates_present :checksum_algorithm, :if => Proc.new { |packet| packet.underlying_ip_version == 6 }, :message => "Checksum algorithm is mandatory when used with IPv6"
      end
