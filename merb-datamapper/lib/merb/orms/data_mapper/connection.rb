    ...
    module DataMapper
      class << self
        ...
        def connection_string
          ...
          when :mysql
            "#{config[:adapter]}://#{username_and_passwd}#{config[:host] || 'localhost'}/#{config[:database]}"
          when :postgresql
            "postgres://#{username_and_passwd}#{config[:host] || 'localhost'}/#{config[:database]}"
          when :sqlite3
            "sqlite3://#{Dir.pwd}/#{config[:database]}"
          end
        end

        def username_and_passwd
