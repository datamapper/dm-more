...
teardown = lambda do
  [ :Dragon, :Country, :Knight ].each { |m| Object.send(:remove_const, m) }
end

module TargetMethods
  def self.included(base)
    base.cattr_accessor :subject_type
  end

  def subject_for(klass)
    self.class.subject_type == DataMapper::Collection ? klass.all : klass
  end
end

shared_examples_for 'count with no arguments' do
  it 'should count the results' do
    subject_for(Dragon).count.should  == 3

    subject_for(Country).count.should == 7
  end

  it 'should count the results with conditions having operators' do
    subject_for(Dragon).count(:toes_on_claw.gt => 3).should == 2

    subject_for(Country).count(:birth_rate.lt => 12).should == 3
    subject_for(Country).count(:population.gt => 1000000000).should == 1
    subject_for(Country).count(:population.gt => 2000000000).should == 0
    subject_for(Country).count(:population.lt => 10).should == 0
  end

  it 'should count the results with raw conditions' do
    dragon_statement = 'is_fire_breathing = ?'
    subject_for(Dragon).count(:conditions => [ dragon_statement, false ]).should == 1
    subject_for(Dragon).count(:conditions => [ dragon_statement, true  ]).should == 2
  end
end
...
shared_examples_for 'An Aggregatable Class' do
  ...
  describe '#count' do
    ...
    describe 'with a property name' do
      it 'should count the results' do
        subject_for(Dragon).count(:name).should == 2
      end

      it 'should count the results with conditions having operators' do
        subject_for(Dragon).count(:name, :toes_on_claw.gt => 3).should == 1
      end

      it 'should count the results with raw conditions' do
        statement = 'is_fire_breathing = ?'
        subject_for(Dragon).count(:name, :conditions => [ statement, false ]).should == 1
        subject_for(Dragon).count(:name, :conditions => [ statement, true  ]).should == 1
      end
    end
  end
  ...
  describe '#min' do
    describe 'with no arguments' do
      it 'should raise an error' do
        lambda { subject_for(Dragon).min }.should raise_error(ArgumentError)
      end
    end

    describe 'with a property name' do
      it 'should provide the lowest value of an Integer property' do
        subject_for(Dragon).min(:toes_on_claw).should == 3
        subject_for(Country).min(:population).should == 82369548
      end

      it 'should provide the lowest value of a Float property' do
        subject_for(Country).min(:birth_rate).should be_kind_of(Float)
        subject_for(Country).min(:birth_rate).should >= 7.87 - @approx_by  # approx match
        subject_for(Country).min(:birth_rate).should <= 7.87 + @approx_by  # approx match
      end

      it 'should provide the lowest value of a BigDecimal property' do
        subject_for(Country).min(:gold_reserve_value).should be_kind_of(BigDecimal)
        subject_for(Country).min(:gold_reserve_value).should == BigDecimal('1217050983400.0')
      end

      it 'should provide the lowest value of a DateTime property' do
        subject_for(Dragon).min(:birth_at).should be_kind_of(DateTime)
        subject_for(Dragon).min(:birth_at).to_s.should == @birth_at.to_s
      end

      it 'should provide the lowest value of a Date property' do
        subject_for(Dragon).min(:birth_on).should be_kind_of(Date)
        subject_for(Dragon).min(:birth_on).to_s.should == @birth_on.to_s
      end

      it 'should provide the lowest value of a Time property' do
        subject_for(Dragon).min(:birth_time).should be_kind_of(Time)
        subject_for(Dragon).min(:birth_time).to_s.should == @birth_time.to_s
      end

      it 'should provide the lowest value when conditions provided' do
        subject_for(Dragon).min(:toes_on_claw, :is_fire_breathing => true).should  == 4
        subject_for(Dragon).min(:toes_on_claw, :is_fire_breathing => false).should == 3
      end
    end
  end
  ...
  describe '#max' do
    describe 'with no arguments' do
      it 'should raise an error' do
        lambda { subject_for(Dragon).max }.should raise_error(ArgumentError)
      end
    end

    describe 'with a property name' do
      it 'should provide the highest value of an Integer property' do
        subject_for(Dragon).max(:toes_on_claw).should == 5
        subject_for(Country).max(:population).should == 1330044605
      end

      it 'should provide the highest value of a Float property' do
        subject_for(Country).max(:birth_rate).should be_kind_of(Float)
        subject_for(Country).max(:birth_rate).should >= 20.04 - @approx_by  # approx match
        subject_for(Country).max(:birth_rate).should <= 20.04 + @approx_by  # approx match
      end

      it 'should provide the highest value of a BigDecimal property' do
        subject_for(Country).max(:gold_reserve_value).should == BigDecimal('22589877164500.0')
      end
      ...
      it 'should provide the highest value when conditions provided' do
        subject_for(Dragon).max(:toes_on_claw, :is_fire_breathing => true).should  == 5
        subject_for(Dragon).max(:toes_on_claw, :is_fire_breathing => false).should == 3
      end
    end
  end
  ...
  describe '#avg' do
    describe 'with no arguments' do
      it 'should raise an error' do
        lambda { subject_for(Dragon).avg }.should raise_error(ArgumentError)
      end
    end

    describe 'with a property name' do
      it 'should provide the average value of an Integer property' do
        subject_for(Dragon).avg(:toes_on_claw).should be_kind_of(Float)
        subject_for(Dragon).avg(:toes_on_claw).should == 4.0
      end

      it 'should provide the average value of a Float property' do
        mean_birth_rate = (13.71 + 14.18 + 16.04 + 11.03 + 7.87 + 20.04 + 8.18) / 7
        subject_for(Country).avg(:birth_rate).should be_kind_of(Float)
        subject_for(Country).avg(:birth_rate).should >= mean_birth_rate - @approx_by  # approx match
        subject_for(Country).avg(:birth_rate).should <= mean_birth_rate + @approx_by  # approx match
      end
      ...
      it 'should provide the average value of a BigDecimal property' do
        ...
        subject_for(Country).avg(:gold_reserve_value).should be_kind_of(BigDecimal)
        subject_for(Country).avg(:gold_reserve_value).should == BigDecimal(mean_gold_reserve_value.to_s)
      end

      it 'should provide the average value when conditions provided' do
        subject_for(Dragon).avg(:toes_on_claw, :is_fire_breathing => true).should  == 4.5
        subject_for(Dragon).avg(:toes_on_claw, :is_fire_breathing => false).should == 3
      end
    end
  end
  ...
  describe '#sum' do
    describe 'with no arguments' do
      it 'should raise an error' do
        lambda { subject_for(Dragon).sum }.should raise_error(ArgumentError)
      end
    end

    describe 'with a property name' do
      it 'should provide the sum of values for an Integer property' do
        subject_for(Dragon).sum(:toes_on_claw).should == 12

        total_population = 1330044605 + 303824646 + 191908598 + 140702094 +
                           127288419 + 109955400 + 82369548
        subject_for(Country).sum(:population).should == total_population
      end

      it 'should provide the sum of values for a Float property' do
        total_tonnes = 600.0 + 8133.5 + 438.2 + 765.2 + 3417.4
        subject_for(Country).sum(:gold_reserve_tonnes).should be_kind_of(Float)
        subject_for(Country).sum(:gold_reserve_tonnes).should >= total_tonnes - @approx_by  # approx match
        subject_for(Country).sum(:gold_reserve_tonnes).should <= total_tonnes + @approx_by  # approx match
      end

      it 'should provide the sum of values for a BigDecimal property' do
        subject_for(Country).sum(:gold_reserve_value).should == BigDecimal('37090059214100.0')
      end

      it 'should provide the average value when conditions provided' do
        subject_for(Dragon).sum(:toes_on_claw, :is_fire_breathing => true).should  == 9
        subject_for(Dragon).sum(:toes_on_claw, :is_fire_breathing => false).should == 3
      end
    end
  end
  ...
  describe '#aggregate' do
    describe 'with no arguments' do
      it 'should raise an error' do
        lambda { subject_for(Dragon).aggregate }.should raise_error(ArgumentError)
      end
    end

    describe 'with only aggregate fields specified' do
      it 'should provide aggregate results' do
        results = subject_for(Dragon).aggregate(:all.count, :name.count, :toes_on_claw.min, :toes_on_claw.max, :toes_on_claw.avg, :toes_on_claw.sum)
    ...
    describe 'with aggregate fields and a property to group by' do
      it 'should provide aggregate results' do
        results = subject_for(Dragon).aggregate(:all.count, :name.count, :toes_on_claw.min, :toes_on_claw.max, :toes_on_claw.avg, :toes_on_claw.sum, :is_fire_breathing)
...
if HAS_SQLITE3 || HAS_MYSQL || HAS_POSTGRES
  describe DataMapper::Model do
    ...
    self.subject_type = DataMapper::Model
  ...
  describe DataMapper::Collection do
    ...
    self.subject_type = DataMapper::Collection
